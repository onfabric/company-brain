...
export const command = defineCommand('cloud doctor', {
  description: 'Check cloud health.',
  options: {
    yes: {
    ...
    const checks: DoctorCheck[] = [
      { label: 'AWS login', ok: true, detail: `${prerequisites.accountId} (${prerequisites.arn})` },
      {
        ...
        detail: config.outputs
          ...
          : 'Run `bun run company-brain cloud setup`.',
      },
    ];
...
async function nangoApiCheck(
  ...
  if (!config.secrets.nangoSecretKey) {
    return {
      ...
      ok: false,
      detail:
        'Open the hosted Nango dashboard, copy the dev API key, then run `bun run company-brain cloud resume`.',
    };
  }
