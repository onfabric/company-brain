...
: "${NANGO_IMAGE_URI:?}" "${BRAIN_IMAGE_URI:?}" "${PG_BACKUP_IMAGE_URI:?}" "${LOGTO_SETUP_IMAGE_URI:?}" "${SSM_SECRET_PREFIX:?}" "${NANGO_HOSTNAME:?}" "${NANGO_CONNECT_HOSTNAME:?}" "${BRAIN_HOSTNAME:?}" "${DOZZLE_HOSTNAME:?}" "${AUTH_HOSTNAME:?}" "${ACME_EMAIL:?}" "${AWS_DEFAULT_REGION:?}" "${DATA_VOLUME_ID:?}" "${ARTIFACTS_BUCKET:?}"
...
LOGTO_SETUP_IMAGE_URI=${LOGTO_SETUP_IMAGE_URI}
