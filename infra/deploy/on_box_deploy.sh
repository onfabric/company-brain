...
# NANGO_HOSTNAME, DOZZLE_HOSTNAME, ACME_EMAIL, AWS_DEFAULT_REGION), exported by
# the SSM command.
set -euo pipefail

cd "$(dirname "$0")"

: "${IMAGE_URI:?}" "${SSM_SECRET_PREFIX:?}" "${NANGO_HOSTNAME:?}" "${DOZZLE_HOSTNAME:?}" "${ACME_EMAIL:?}" "${AWS_DEFAULT_REGION:?}"
