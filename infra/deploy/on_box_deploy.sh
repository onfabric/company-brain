...
# NANGO_CONNECT_HOSTNAME, USAGE_DASHBOARD_HOSTNAME, DOZZLE_HOSTNAME, ACME_EMAIL,
...
: "${NANGO_IMAGE_URI:?}" "${BRAIN_IMAGE_URI:?}" "${USAGE_DASHBOARD_IMAGE_URI:?}" "${SSM_SECRET_PREFIX:?}" "${NANGO_HOSTNAME:?}" "${NANGO_CONNECT_HOSTNAME:?}" "${USAGE_DASHBOARD_HOSTNAME:?}" "${DOZZLE_HOSTNAME:?}" "${ACME_EMAIL:?}" "${AWS_DEFAULT_REGION:?}"
