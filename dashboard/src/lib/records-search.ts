import type { RecordFilesystemInput, RecordsQueryInput } from '#/lib/brain-functions.ts';
import {
  API_MAX_LIMIT,
  DASHBOARD_TABS,
  DATE_SLICE_END,
...
export function normalizeRouteSearch(search: Record<string, unknown>): RecordsRouteSearch {
  return {
    ...
    fsDataSourceId: stringValue(search.fsDataSourceId),
    fsDay: stringValue(search.fsDay),
    fsPersonId: stringValue(search.fsPersonId),
  };
}
...
export function toRecordFilesystemInput(
  search: RecordsRouteSearch,
): Omit<RecordFilesystemInput, 'offset'> {
  return {
    dataSourceId: search.fsDataSourceId,
    day: search.fsDay,
    personId: search.fsPersonId,
    limit: search.limit,
  };
}
...
export function cleanRouteSearch(search: RecordsRouteSearch): RecordsRouteSearch {
  return {
    ...
    fsDataSourceId: search.fsDataSourceId || undefined,
    fsDay: search.fsDay || undefined,
    fsPersonId: search.fsPersonId || undefined,
  };
}
