...
export function launchAgentConfig(dataDir: string): LaunchAgentConfig {
...
function daemonProgramArguments(): string[] {
  const scriptPath = process.argv[1];
  if (scriptPath && scriptPath !== process.execPath && scriptPath.endsWith('.ts')) {
    return [process.execPath, path.resolve(scriptPath), 'daemon'];
  }

  const cliPath = fileURLToPath(new URL('./cli.ts', import.meta.url));
  if (fs.existsSync(cliPath)) {
    return [process.execPath, cliPath, 'daemon'];
  }
