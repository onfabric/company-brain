...
- Row types: query result types are generated by `@ilbertt/bun-sqlgen` into `src/db/queries.gen.d.ts` — never hand-write them. Name each query on the `sql` tag (`` this.sql.SelectKnowledge`…` ``) and read its row type from `QueryResults['SelectKnowledge']`; query params are plain primitives. Columns the engine can't infer get an inline hint in the query — `/* @type col <TS type> */` for `json`/computed columns, `/* @notNull col */` for generated columns the catalog reports as nullable (or a `COMMENT ON COLUMN … @notNull` in a migration, as for uuidv7-derived `created_at`). Repeated `@type` shapes (e.g. the participants aggregate) reference one exported type via `import('#…').Foo` so the shape stays single-sourced. A query whose body can't be statically planned — a dynamic CTE over a non-introspected schema like `nango_records` — stays hand-typed as `` tx<Row[]>`…` `` rather than a tag. Run `bun codegen:sql` after changing a query or migration and commit the regenerated output; CI enforces it via `bun codegen:sql:check`.
