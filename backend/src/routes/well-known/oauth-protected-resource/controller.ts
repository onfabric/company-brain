...
// brain itself, which hosts the AS metadata, /token, and /oidc/register.
// Served at both the root and the path-suffixed form (clients try
// `/.well-known/oauth-protected-resource/mcp` first). Unauthenticated by design.
const METADATA: Static<typeof ProtectedResourceMetadataSchema> = {
  resource: env.mcpResource.href,
  authorization_servers: [env.brainPublicUrl.origin],
