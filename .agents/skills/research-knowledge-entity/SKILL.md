...
1. **Cast a wide net first.** Search `records` and `people` for the entity under *every* spelling, handle, email, alias, nickname, and former name. People change emails and surnames; companies rebrand; products get renamed mid-life. Missing an alias means missing half the story.
2. **Read the strongest sources in full.** Don't distill from snippets. Open the meetings, threads, and documents that mention the entity most or most substantively and read them properly. The interesting facts — a decision, a fallout, a pivot — are usually in the body, not the subject line.
3. **Mine every kind of source.** The raw material spans meeting notes and transcripts, Slack discussions, email exchanges, calendar events, GitHub activity, Notion docs, and more. Different source types answer different questions: meetings and email reveal relationships and what was said; GitHub and Notion reveal what was actually built or written; Slack reveals day-to-day reality and tone. Cross-check them — what someone said in a meeting and what landed in GitHub are both part of the truth.
4. **Follow the leads.** Every source surfaces new threads: a name you haven't covered, a project you didn't know about, a date that contradicts another, a company on the other side of a deal. Pull each thread. Research is iterative — each pass should raise new questions that the next pass answers.
5. **Form your own questions.** The starting points below are seeds, not a checklist. Once you know the entity, you will know what *else* matters about it — ask those questions too, and chase them down. An entry that only answers the seed questions hasn't been researched, only filled in.
6. **Keep the evidence trail.** Hold onto the `record` ids and `person` ids behind every claim — they become the entry's source records and participants, and they let any reader trace a statement back to where it came from.
7. **Know when you're done.** You have enough when new sources stop changing the picture — when you can narrate the entity's arc end to end, you understand how it connects to the rest of the brain, and the open questions left are genuinely unanswerable from the material (not just unsearched). If a major phase is a blank — a person with no described work, a deal with no outcome, a product with no status — you are not done.
...
- How did Fabric engage with it, and how did that engagement evolve? Was there a specific project or deal? Who were the key people on both sides? What meetings happened (Notion, calendar) and what were they about? Any relevant email threads or Slack discussions?
