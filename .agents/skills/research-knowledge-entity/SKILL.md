...
- **Present it visually, not as a wall of text.** A complete picture told as dense paragraphs still fails the reader. Render the synthesis with the component kit from [build-knowledge-base](../build-knowledge-base/SKILL.md) (step 5): a `lede` opener, `stat-grid` cards for headline figures, an `ol.timeline` for the entity's arc (a customer/partner especially suits the *status/proof/outcome cards + origination→status timeline + what-worked/went-wrong/took-away cards* retrospective), `table`/`dl` for structured facts, and `.tag` chips for era/status. Keep stat-card values short; favour a scannable layout over long prose wherever it carries the same information.
