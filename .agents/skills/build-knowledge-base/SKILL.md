...
- **Be visual, not a wall of text — use the component kit.** A page that hits every fact but reads as dense paragraphs is a failure of presentation. The renderer ships a small CSS kit keyed on class names (no inline styles needed); reach for it so each entry is *scannable* like an editorial wiki page. The components (all sanitizer-safe — plain `div`/`span`/`ol`/`p` with these classes):
  - **Lede & intro:** open with `<p class="lede">` (one strong sentence), then an optional `<p class="meta-line">short · dotted · facts</p>`, then `<hr class="rule">`.
  - **Stat cards:** `<div class="stat-grid"><div class="stat"><div class="k">LABEL</div><div class="v">VALUE</div><div class="d">small desc</div></div>…</div>` — 3–4 headline figures. Variants `<div class="stat accent">` (green value) and `<div class="stat warn">` (red, for ended/negative). **Keep each `.v` SHORT** — a number or one–two words; put long repo paths, URLs or slugs in the `.d` description or the `.meta-line`, never as a long unbroken `.v` (it crowds the card).
  - **Timeline** for any arc/chronology: `<ol class="timeline"><li class="is-start"><span class="tag blue">WHEN</span><p class="t-title">…</p><p class="t-body">…</p></li> … <li class="is-end"><span class="tag red">WHEN</span>…</li></ol>` — mark the first item `is-start` (blue dot) and a terminal item (a pause, departure, wind-down, "status today") `is-end` (red dot).
  - **Cards** for parallel points (e.g. *what worked / what went wrong / what we learned*, or grouped facets): `<div class="cards"><div class="card"><h3>…</h3><p>…</p><p class="sub">muted note</p></div>…</div>`.
  - **Tables** for reference/structured data; **`<dl>`** for identifier/fact lists; **`.tag`** chips (`.blue` / `.red` / `.gray`) for era/status labels; `<mark>` to highlight a single key metric.
  - Per type, a good default: a **Person** = lede + stat cards (role / tenure / background / status) + an involvement timeline + a fact `<dl>`; a **Company** (customer/partner) = the retrospective format (status/proof/outcome stat cards + an origination→status timeline + a *what worked / went wrong / took away* card set); a **Product/project** = lede + stat cards (repo / lifespan / era / status) + a milestone timeline + concise prose. Don't force a component where plain prose genuinely reads better, and never drop or distort a fact for layout.
