...
- Create the entry, attaching the `person_ids` and `record_ids` you collected as participants and source records.
- **Have the user review it.** Share the new entry's `html_url` (its rendered page) and ask the user to review it; apply their corrections before moving on. A new entry is not "done" until the user has seen and approved it.
...
- **Have the user review the changes.** Before treating the iteration as complete, show the user the modifications you made to existing entries (and the index) — share the affected `html_url`s — and apply their feedback. Edits to entries the user already approved get reviewed just like new ones.
